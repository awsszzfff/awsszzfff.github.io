...
![image-20220903152144268](attachments/image-20220903152144268.png)

访问是一个登录页面，并根据提示信息“后台密码5位弱口令”，弱口令登录进入后台。

![image-20220903152750787](attachments/image-20220903152750787.png)
...
![image-20220903155837001](attachments/image-20220903155837001.png)

下载链接的后半部分可以看出是一串 base64 位编码的字符串，用工具进行解码。

![image-20220903160241785](attachments/image-20220903160241785.png)

![image-20220903153833702](attachments/image-20220903153833702.png)
...
![image-20220903161802365](attachments/image-20220903161802365.png)

**方式二**

设计 -> 高级，有可修改执行的 php 代码，但保存时提示需要服务器存在`/var/www/html/system/tmp/ikuh.txt`文件。

![image-20220903164355204](attachments/image-20220903164355204.png)
...
![image-20220903165421803](attachments/image-20220903165421803.png)

修改文件名称时，看 wp 直接修改为这样的路径，尝试了其他路径也均会失败。

> 此处作为小白的猜想：（做题属实是没想到，对 web 开发或 linux 目录结构还是不太了解）
>
> ![image-20220903172101574](attachments/image-20220903172101574.png)
>
> 想到上方所导出的主题文件，打开可以看到目录结构
>
> ![image-20220903172224999](attachments/image-20220903172224999.png)
>
> 根据这个目录结构修改文件，实现目录穿越。

最终修改的文件名称：

![image-20220903170123299](attachments/image-20220903170123299.png)

此时再回过头来重新保存 PHP 代码。刷新原主页便可得到 flag 。

![image-20220903170329295](attachments/image-20220903170329295.png)