    ...
    def threeSumClosest(self, nums: List[int], target: int) -> int:
        ...
        for i in range(n - 2):
            ...
            while j < k:
                ...
                if s == target:
                    return s
                if abs(ans - target) > abs(s - target):	# 不同的比较方式
                    ans = s
                if s > target:
                    k -= 1
                else:
    ...
    def zigzagLevelOrder(self, root: Optional[TreeNode]) -> List[List[int]]:
        ...
        while q:
            ...
            for _ in range(len(q)):
                ...
                if node.right: q.append(node.right)
            flag = not flag
            ans.append(vals if flag else vals[::-1])
...
class Solution:
    def combine(self, n: int, k: int) -> List[List[int]]:
        ...
        def dfs(i):
            need = k - len(path)  # 剩余需要递归判断的个数
            # if i < need:
            #     return
            if len(path) == k:
                ans.append(path.copy())
                return
            for j in range(i, need - 1, -1):  # 若递归判断个数不够组成k个数，则不再递归
                path.append(j)
                dfs(j - 1)
                path.pop()
...
class Solution:
    def combinationSum3(self, k: int, n: int) -> List[List[int]]:
        ...
        def dfs(i, target):
            need = k - len(path)
            if target < 0 or target > (i + i - (need - 1)) * need // 2:
                return
            if need == 0:
                ans.append(path.copy())
                return
            for j in range(i, need - 1, -1):
                path.append(j)
                dfs(j - 1, target - j)
                path.pop()
...
class Solution:
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        # 确定背包容量，将取正号元素和p 与 取负号元素和q / 2 作为背包容量（通过数学运算推出）
